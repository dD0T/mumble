...
static int trylock(const MumblePIDLookup lookupFunc, const MumblePIDLookupContext lookupContext) {
	...
	if (! initialize(lookupFunc, lookupContext, L"bf4_x86.exe")) { // Link the game executable
		return false;
	}
...
static MumblePlugin bf4plug = {
	...
	MumbleInitConstWideString(L"1.8.2.48475 (x86)"),
	MumbleInitConstWideString(L"Supports Battlefield 4 (x86) with context and identity support."),
	fetch,
	trylock,
	generic_unlock,
};
