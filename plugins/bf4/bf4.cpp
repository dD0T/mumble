...
static int trylock(const MumblePIDLookup lookupFunc, const MumblePIDLookupContext lookupContext) {
	...
	if (! initialize(lookupFunc, lookupContext, L"bf4.exe")) { // Link the game executable
		return false;
	}
...
static MumblePlugin bf4plug = {
    ...
    MumbleInitConstWideString(L"1.8.2.48475 (x64)"),
    MumbleInitConstWideString(L"Supports Battlefield 4 (x64) with context and identity support."),
    fetch,
    trylock,
    generic_unlock,
};
