...
bool AudioOutput::mix(void *outbuff, unsigned int nsamp) {
	...
	if (! qlMix.isEmpty()) {
		...
		if (g.s.bPositionalAudio
		    ...
		    && g.p->fetch()
		    && (g.bPosTest
		        || fabs(g.p->fCameraPosition[0]) > 0.0f
		        || fabs(g.p->fCameraPosition[1]) > 0.0f
		        || fabs(g.p->fCameraPosition[2]) > 0.0f)) {
		...
		foreach(AudioOutputUser *aop, qlMix) {
			...
			float prioritySpeakerVolumeAdjustmentFactor = 1;
			...
			if (speech) {
				...
				prioritySpeakerVolumeAdjustmentFactor *= user->fLocalVolume;
				if (prioritySpeakerActive) {
					if (user->tsState != Settings::Whispering
					    && !user->bPrioritySpeaker) {
						
						prioritySpeakerVolumeAdjustmentFactor *= adjustFactor;
					}
				}
			}
			...
			if (recorder) {
				...
				if (aos) {
					for (unsigned int i = 0; i < nsamp; ++i) {
						recbuff[i] += pfBuffer[i] * prioritySpeakerVolumeAdjustmentFactor;
					}
			...
			if (validListener
			    && (fabs(aop->fPos[0]) > 0.0f
			        || fabs(aop->fPos[1]) > 0.0f
			        || fabs(aop->fPos[2]) > 0.0f)) {
			...
			if (srcData) {
				...
				for (unsigned int chanIndex=0; chanIndex < nchan; ++chanIndex) {
					const float volumeAdjFactor = defaultVolume * calcGain(chanIndex, *srcData) * prioritySpeakerVolumeAdjustmentFactor;
				...
				for (unsigned int s=0;s<nchan;++s) {
					const float volumeAdjFactor = defaultVolume * prioritySpeakerVolumeAdjustmentFactor;
